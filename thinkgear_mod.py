...
class ThinkGearProtocol(object):
    ...
    def __init__(self, device):
        ...
        self.io = socket.socket(socket.AF_BLUETOOTH, socket.SOCK_STREAM, socket.BTPROTO_RFCOMM)
        self.io.bind((device, 2))
...
class ThinkGearEEGPowerData(ThinkGearData):
    ...
    _decode = staticmethod(
        lambda v: EEGPowerData(*struct.unpack('>8L', ''.join('\x00' + v[o:o + 3] for o in xrange(0, 24, 3)))))
...
def main():
    ...
    print('start')
    for pkt in ThinkGearProtocol('0D:00:18:A1:54:15').get_packets():
